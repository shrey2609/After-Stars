...
const ClaimedFood = require("../model/CLaimed");
...
const addFood = async (req, res, next) => {
  ...
  try {
    ...
    existingUser.claimedFoods.push(food);
...
const updateFood = async (req, res, next) => {
  ...
  let food;
  try {
    food = await ClaimedFood.findByIdAndUpdate(
      foodId,
...
const getById = async (req, res, next) => {
  ...
  let food;
  try {
    food = await ClaimedFood.findById(id);
  } catch (e) {
...
const deleteFood = async (req, res, next) => {
  ...
  let food;
  try {
    food = await ClaimedFood.findByIdAndDelete(id).populate("user");
    if (!food) {
      return res.status(404).json({ message: "Food not found" });
    }
...
const getByUserId = async (req, res, next) => {
  ...
  let userFoods;
  try {
    userFoods = await User.findById(userId).populate("donatedFoods");
  } catch (err) {
  ...
  return res.status(200).json({ user: userFoods });
};
